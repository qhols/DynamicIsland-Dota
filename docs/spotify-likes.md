...
1. Полностью закрой Spotify: ПКМ по его иконке в трее (справа внизу, возле часов) и **Выйти**. Крестик на окне его только прячет
...
**Spotify сам стартует вместе с Windows.** Автозапуск идёт мимо ярлыков, поэтому флага там нет. Либо выключи его в Spotify: Настройки > Запуск и поведение окна > Автоматически открывать Spotify после входа в систему: **Нет**. Либо после старта Windows один раз выйди из Spotify и открой его ярлыком.
